        ...
        <section id="universities" ref={ref} className={`space-y-4 scroll-hidden ${isVisible ? 'scroll-visible' : ''}`}>
            <div className="flex flex-col gap-3 px-6 lg:px-10">
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h2 className="text-3xl font-bold text-[#1F2937]">Mentee ChinaHack đang theo học tại đâu?</h2>
                    ...
                    {repeated.map((item, index) => (
                        ...
                        >
                            <span className="rounded-full bg-violet-100 p-2 text-violet-700">
                                {index % 2 === 0 ? <GraduationCap className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
                            </span>
                            {item.name}
                        </div>
                    ))}
